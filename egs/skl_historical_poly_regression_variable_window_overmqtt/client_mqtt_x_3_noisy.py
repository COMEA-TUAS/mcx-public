...
number_of_samples = len(time_values)

data =  1/5 * (np.power((time_values-1), 3) - np.power((time_values-1), 2) - time_values - 1) + 1/10 * (np.random.rand(number_of_samples))
data = np.insert(data.reshape(number_of_samples,1), 0, time_values, axis=1)
print(data.shape)
data = pd.DataFrame(data, columns =['time', 'A'])
