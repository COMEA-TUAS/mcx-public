...
class FMUModelHandler(ModelHandler):
    ...
    def spawn(self):
        ...
        self._fmu.instantiate(loggingOn=True)
...
class HistoricalModelHandler(ModelHandler):
    def _build_response(self, r, control_vals=None):
        """Attach variable names to response list"""
        keys = self.input_keys + self.target_keys
        response = {k: v for k, v in zip(keys, r)}
...
class FixedLengthHistoricalRegressionModelHandler(HistoricalModelHandler):
    def __init__(self, sources=None, input_keys=None, target_keys=None,
                 control_keys=None, window_size=-1, step_size=1):
        super().__init__(sources=sources,
                         input_keys=input_keys,
                         target_keys=target_keys,
                         control_keys=control_keys)
    ...
    def step(self):
        """ Calculates (linearly predicts) the next state according to what is stored in 
            the self.buffer
        ...
        if len(self.buffer) < self.window_size:
            y_pred = [0] * len(self.target_keys)
        else:
            self.status = ModelStatus.BUSY
            X = []
            Y = []
            if self.control_keys:
                control_vals = {k: self.buffer[0][k] for k in self.control_keys}
            else:
                control_vals = None
            for index, y in enumerate(self.buffer):
                
                x = self.step_size * index
                if isinstance(y, dict):
                    y = Measurement(y)
                for k, v in y.items():
                    if v is None:
                        y[k] = 0
                y = y.to_numpy(self.input_keys)
                Y.append(y[0])
                X.append([x])
            Y = np.asarray(Y)
            X = np.asarray(X)
            ...
            try:
                ...
                y_pred = regr.predict([[self.step_size * self.window_size]])
                # input + target
                ...
                raise ModelStepException(
                    "Could not get a valid response from model")
        print(y_pred)
        result = list(Y[0]) + list(y_pred[0])
        result = self._build_response(result, control_vals)
        self.status = ModelStatus.READY
        return result

class VariableLengthHistoricalRegressionModelHandler(HistoricalModelHandler):
    def __init__(self, sources=None, input_keys=None, target_keys=None,
                 control_keys=None, min_window_size=2, step_size=1):
        super().__init__(sources=sources,
                         input_keys=input_keys,
                         target_keys=target_keys,
                         control_keys=control_keys)
    ...
    def step(self, n_degree=1):
        """ fits a polynomial with degree of `n_degree` to the most recent received datapoints
            and then predicts the next timestep value according to that polynomial.
        ...
        if self.control_keys:
            ...
            control_vals = None
        time_pred = X[0][0] + self.step_size * len(self.buffer)
        self.buffer = []

        time.sleep(self.step_size * 100 * random.random())
        try:
            polyreg = make_pipeline(PolynomialFeatures(n_degree), linear_model.LinearRegression())
            polyreg.fit(X,Y)
            y_pred = polyreg.predict([[time_pred]])
        except Exception:
            raise ModelStepException(
                "Could not get a valid response from model")
        #print(X.shape)
        ...
        y_pred.append(time_pred)
        # print(y_pred)
        result = list(Y[0]) + y_pred
        # print(result)
        result = self._build_response(result, control_vals)
        self.status = ModelStatus.READY
        return result
